...
static unsigned int *create_dimensions(unsigned int total_dimension,
                                       unsigned int count) {
...
static unsigned int *create_offsets(unsigned int *dimensions,
                                    unsigned int count) {
...
cart2d_t *cart2d_create(unsigned int grid_x, unsigned int grid_y,
...
int cart2d_set_grid(cart2d_t *cart, unsigned int x, unsigned int y,
                    grid_t *grid) {
  if (cart == NULL || x >= cart->grid_x_count || y >= cart->grid_y_count) {
    LOG_ERROR("invalid input arguments");
    goto fail_exit;
...
cart2d_t *cart2d_from_grid(grid_t *grid, unsigned int grid_x,
                           unsigned int grid_y) {
  if (grid == NULL) {
    LOG_ERROR_NULL_PTR();
    goto fail_exit;
  ...
  for (int j = 0; j < cart->grid_y_count; j++) {
    for (int i = 0; i < cart->grid_x_count; i++) {
      ...
      if (grid_copy_block(grid, x_offset, y_offset, x_size, y_size, dst_grid, 0,
                          0) < 0) {
...
grid_t *cart2d_to_grid(cart2d_t *cart) {
  ...
  for (int j = 0; j < cart->grid_y_count; j++) {
    for (int i = 0; i < cart->grid_x_count; i++) {
      ...
      if (grid_copy_block(src_grid, 0, 0, x_size, y_size, grid, x_offset,
                          y_offset) < 0) {
        LOG_ERROR("failed to copy grid block");
        goto fail_destroy_grid;
      }
