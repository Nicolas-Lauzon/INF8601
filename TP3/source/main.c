...
static void fail_number_conversion(const char *exec_name, const char *num,
                                   const char *opt) {
...
static void fail_negative_arg(const char *exec_name, const char *opt,
                              const char *arg) {
...
void convert_and_set_positive_arg(const char *exec_name, const char *opt,
                                  const char *arg, unsigned int *result) {
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp("--iterations", argv[i]) == 0) {
      ...
      convert_and_set_positive_arg(exec_name, argv[i], argv[i + 1],
                                   &iterations);
      i++;
