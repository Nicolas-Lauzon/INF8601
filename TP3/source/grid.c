...
grid_t *grid_create(unsigned int width, unsigned int height,
  ...
  grid->data =
      calloc(grid->width_padded * grid->height_padded, sizeof(*grid->data));
  if (grid->data == NULL) {
    LOG_ERROR_ERRNO("malloc");
    goto fail_malloc2;
  }
...
static int grid_assert_equal_dimensions(grid_t *grid, grid_t *other) {
  ...
  /*
          ...
          if (grid->height_padded != other->height_padded || grid->width_padded
     != other->width_padded) { LOG_ERROR("grid padded dimensions are
     different"); goto fail_exit;
          }
  */
  return 0;
...
int grid_copy_block(grid_t *src, unsigned int x1, unsigned int y1,
                    unsigned int w, unsigned int h, grid_t *dst,
                    unsigned int x2, unsigned int y2) {
  if (x1 + w > src->width || y1 + h > src->height) {
    LOG_ERROR("invalid source bounds (%u > %u) || (%u > %u)", x1 + w,
              src->width, y1 + h, src->height);
    goto fail_exit;
  }

  if (x2 + w > dst->width || y2 + h > dst->height) {
    LOG_ERROR("invalid destination bounds (%u > %u) || (%u > %u)", x2 + w,
              dst->width, y2 + h, dst->height);
    goto fail_exit;
  }
...
int grid_copy_inner_border(grid_t *src, grid_t *dst) {
  if (src->width != dst->width) {
    LOG_ERROR("width mismatch between the grids (src=%u, dst=%u)", src->width,
              dst->width);
    goto fail_exit;
  }

  if (src->height != dst->height) {
    LOG_ERROR("height mismatch between the grids (src=%u, dst=%u)", src->height,
              dst->height);
    goto fail_exit;
  }
  ...
  for (int i = 0; i < src->width; i++) {
    ...
    *grid_get_cell(dst, i, dst->height - 1) =
        *grid_get_cell(src, i, src->height - 1);
  }

  for (int j = 0; j < src->height; j++) {
    *grid_get_cell(dst, 0, j) = *grid_get_cell(src, 0, j);
    *grid_get_cell(dst, dst->width - 1, j) =
        *grid_get_cell(src, src->width - 1, j);
  }
