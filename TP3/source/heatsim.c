...
int heatsim_diffuse(grid_t *current, grid_t *next) {
  ...
  if (current->width_padded != next->width_padded ||
      current->height_padded != next->height_padded) {
...
static cart2d_t *load_image(char *filename, unsigned int dim_x,
                            unsigned int dim_y) {
...
int heatsim_run(char *input, char *output, unsigned int dim_x,
                unsigned int dim_y, unsigned int iterations) {
  heatsim_t heatsim;
  cart2d_t *cart = NULL;
  ...
  printf("[%d] (x,y)=(%u,%u), (S,W,N,E)=(%d,%d,%d,%d)\n", heatsim.rank,
         ...
         heatsim.rank_south_peer, heatsim.rank_west_peer,
         heatsim.rank_north_peer, heatsim.rank_east_peer);
  ...
  if (heatsim.rank == 0) {
    ...
    grid_t *current_grid_no_padding =
        cart2d_get_grid(cart, heatsim.coordinates[0], heatsim.coordinates[1]);
    if (current_grid_no_padding == NULL) {
      LOG_ERROR("simulation failed to get grid at (%d, %d) (rank 0)",
                heatsim.coordinates[0], heatsim.coordinates[1]);
      goto fail_destroy_cart;
    }
  ...
  printf("[%d] grid config: padding=%u width=%d height=%d\n", heatsim.rank,
         current_grid->padding, current_grid->width, current_grid->height);
