...
int heatsim_init(heatsim_t *heatsim, unsigned int dim_x, unsigned int dim_y) {
  ...
  ret = MPI_Cart_create(MPI_COMM_WORLD, 2, dimensions, periods, false,
  ...
  if (ret != MPI_SUCCESS) {
    LOG_ERROR_MPI("MPI_Comm_rank : ", ret);

    goto fail_exit;
  }

  ret = MPI_Cart_shift(heatsim->communicator, 1, 1, &heatsim->rank_north_peer,
  ...
  ret = MPI_Cart_shift(heatsim->communicator, 0, 1, &heatsim->rank_west_peer,
  ...
  ret = MPI_Cart_coords(heatsim->communicator, heatsim->rank, 2,
                        heatsim->coordinates);
  if (ret != MPI_SUCCESS) {
...
// Not necessary, we couldve just sent grid->data
...
MPI_Datatype grid_size_struct() {
  ...
  MPI_Datatype field_types[3] = {MPI_UNSIGNED, MPI_UNSIGNED, MPI_UNSIGNED};

  MPI_Aint field_positions[3] = {offsetof(dim_t, width),
...
int heatsim_send_grids(heatsim_t *heatsim, cart2d_t *cart) {
  ...
  for (int i = 1; i < heatsim->rank_count; i++) {
    ...
    struct dimensions dim_to_send = {grid->width, grid->height, grid->padding};

    ret = MPI_Isend(&dim_to_send, 1, message_type, i, 3, heatsim->communicator,
                    &request);
    if (ret != MPI_SUCCESS) {
    ...
    ret = MPI_Isend(grid->data, 1, data_type, i, 4, heatsim->communicator,
                    &request);
    if (ret != MPI_SUCCESS) {
...
grid_t *heatsim_receive_grid(heatsim_t *heatsim) {
  ...
  MPI_Request request;
  ret = MPI_Irecv(&buf, 1, message_type, 0, 3, heatsim->communicator, &request);
  if (ret != MPI_SUCCESS) {
  ...
  MPI_Datatype data_type =
      grid_data_struct(newGrid->width_padded * newGrid->height_padded);

  ret = MPI_Irecv(newGrid->data, newGrid->width_padded * newGrid->height_padded,
...
int heatsim_exchange_borders(heatsim_t *heatsim, grid_t *grid) {
  ...
  MPI_Datatype type;
  ret = MPI_Type_vector(height, 1, grid->width_padded, MPI_DOUBLE, &type);
  if (ret != MPI_SUCCESS) {
  ...
  // RECEIVE WEST BORDER
  ret = MPI_Irecv(grid_get_cell(grid, -1, 0), 1, type, heatsim->rank_west_peer,
                  0, heatsim->communicator, &request[0]);
  if (ret != MPI_SUCCESS) {
  ...
  ret = MPI_Irecv(grid_get_cell(grid, 0, height), width, MPI_DOUBLE,
                  heatsim->rank_south_peer, 1, heatsim->communicator,
                  &request[1]);
  if (ret != MPI_SUCCESS) {
  ...
  ret = MPI_Irecv(grid_get_cell(grid, 0, -1), width, MPI_DOUBLE,
                  heatsim->rank_north_peer, 2, heatsim->communicator,
                  &request[2]);
  if (ret != MPI_SUCCESS) {
  ...
  // RECEIVE EAST BORDER
  ret =
      MPI_Irecv(grid_get_cell(grid, width, 0), 1, type, heatsim->rank_east_peer,
                3, heatsim->communicator, &request[3]);
  if (ret != MPI_SUCCESS) {
  ...
  // SEND EAST BORDER
  ret =
      MPI_Isend(grid_get_cell(grid, width - 1, 0), 1, type,
                heatsim->rank_east_peer, 0, heatsim->communicator, &request[4]);
  if (ret != MPI_SUCCESS) {
  ...
  // SEND NORTH BORDER
  ret = MPI_Isend(grid_get_cell(grid, 0, 0), width, MPI_DOUBLE,
                  heatsim->rank_north_peer, 1, heatsim->communicator,
                  &request[5]);
  if (ret != MPI_SUCCESS) {
  ...
  // SEND SOUTH BORDER
  ret = MPI_Isend(grid_get_cell(grid, 0, height - 1), width, MPI_DOUBLE,
                  heatsim->rank_south_peer, 2, heatsim->communicator,
                  &request[6]);
  if (ret != MPI_SUCCESS) {
  ...
  // SEND WEST BORDER
  ret = MPI_Isend(grid_get_cell(grid, 0, 0), 1, type, heatsim->rank_west_peer,
                  3, heatsim->communicator, &request[7]);
  if (ret != MPI_SUCCESS) {
  ...
  if (ret != MPI_SUCCESS) {
    ...
    printf("MPI process %d from rank %d, with tag %d and error code %d.\n",
           heatsim->rank, status->MPI_SOURCE, status->MPI_TAG,
           status->MPI_ERROR);
    goto fail_exit;
  }
...
int heatsim_send_result(heatsim_t *heatsim, grid_t *grid) {
  ...
  ret = MPI_Isend(grid->data, grid->width * grid->height, MPI_DOUBLE, 0, 1,
                  heatsim->communicator, &requests);
  if (ret != MPI_SUCCESS) {
...
int heatsim_receive_results(heatsim_t *heatsim, cart2d_t *cart) {
  ...
  for (unsigned int i = 1; i < heatsim->rank_count; i++) {
    ...
    ret = MPI_Irecv(grid->data, grid->width * grid->height, MPI_DOUBLE, i, 1,
                    heatsim->communicator, &request);
    if (ret != MPI_SUCCESS) {
